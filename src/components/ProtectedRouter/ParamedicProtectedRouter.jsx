...
export default function ParamedicProtectedRouter({ children }) {
  ...
  if (user && (user.Type === 'Admin' || user.Type === 'Hospital')) {
  ...
  if (user && user.Type === 'Paramedic' && user.inService === 'NotActive') {
    return (<>
      <Navigate to='/'></Navigate>
      {toast.error('You Are Not Active Paramedic', {
        position: "top-right",
        autoClose: 3800,
        hideProgressBar: false,
        closeOnClick: true,
